import {
  ...
  import { ImageAnalysisResponseDto } from 'src/modules/deepseek-r1/dtos/equipment.dto';
  @Injectable()
  export class ResponseInterceptor implements NestInterceptor {
    ...
    async intercept(context: ExecutionContext, next: CallHandler): Promise<any> {
      ...
      if (classSerialization?.name === ImageAnalysisResponseDto?.name) {
        const getData = body as ImageAnalysisResponseDto;
        const translatedMessage = this.i18nService.translate('Operation successful.', {
          lang: request.headers['accept-language'] || 'en',
          defaultValue: 'Operation successful.',
        });
        const responseData = plainToInstance(classSerialization, { ...getData, message: translatedMessage });
        return of({
          statusCode: status,
